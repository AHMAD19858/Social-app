...
const { getPosts, getuserPosts } = usePostStore();
const { getUsers, getSinglUser } = useUsersStore();
const { getcommnts } = useCommentStore();
const { posts, userPosts } = storeToRefs(usePostStore());
const { users, user } = storeToRefs(useUsersStore());
...
const hotPost = ref({});
getuserPosts(route.params.id).then(() => {
  getPosts().then(() => {
    userPosts.value = userPosts.value.map((e) => {
      const postUser = users.value.find((user) => user.id === e.userId);
      const postComments = comments.value.filter(
        (comment) => comment.postId === e.id
      );
  ...
  <div >
    ...
    >
      ...
      >
          ...
          <NuxtLink  :to="`/`" class="flex h-16 shrink-0 items-center gap-2">
          <img
            class="h-8 w-auto"
            src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=500"
            alt="Your Company"
        ...
        >
          ...
          <div class="flex items-start gap-4">
            <img
              ...
              :src="
               post.postUser?.image ?? 'https://source.unsplash.com/40x40/?portrait?4'
              "
            />
            <div>
          ...
          <p class="text-base font-medium my-1 pt-2 dark:text-white font-montse">{{ post.title }}</p>
          ...
          <div class="flex items-center justify-between mt-4 dark:text-white">
            <div class="flex gap-1">
              <div class="bg-blue-500 rounded-full px-1 dark:text-white">
                <HandThumbUpIcon class="text-white h-5 w-4 pt-1 dark:text-white" />
              </div>
              <p class="dark:text-white">{{ post.reactions.likes }}</p>
            </div>
            <!-- views -->
            <div class="flex items-center gap-1 dark:text-white">
              <EyeIcon class="h-5 w-4" />
              <p class="dark:text-white">{{ post.views }}</p>
            </div>
          </div>
          <!-- actions -->
          <section
            class="w-full flex items-center justify-around gap-8 border-t border-b dark:border-gray-700 my-4 py-1 dark:text-white"
          >
            <div
              @click="like(post)"
            ...
            <div
              ...
              class="flex items-center gap-2 cursor-pointer"
            >
              <ShareIcon class="h-4 w-4 mt-[2px] dark:text-white" />
          ...
          <!-- comments -->
          <div
            class="animate-slideDown px-4 border dark:border-gray-700 rounded-lg flex justify-between items-center gap-2 w-full"
